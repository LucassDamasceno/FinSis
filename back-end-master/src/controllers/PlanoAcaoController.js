...
const create = async (req, res) => {
    try {
        const planoAcao = await models.Plano_Acao.create(req.body);
...
const getAll = async (req, res) => {
    try {
        const planoAcoes = await models.Plano_Acao.findAll();
...
const getById = async (req, res) => {
    try {
        ...
        const planoAcao = await models.Plano_Acao.findOne({
...
const update = async (req, res) => {
    try {
        const { planoAcaoId } = req.params;
        const [updated] = await models.Plano_Acao.update(req.body, {
            where: { id: planoAcaoId }
        });
        if (updated) {
            const updatedPlanoAcao = await models.Plano_Acao.findOne({ where: { id: planoAcaoId } });
...
const destroy = async (req, res) => {
    try {
        const { planoAcaoId } = req.params;
        const deleted = await models.Plano_Acao.destroy({
            where: { id: planoAcaoId }
        });
        if (deleted) {
